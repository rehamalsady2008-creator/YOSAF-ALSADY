      ...
      >
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          <div className="space-y-4">
            ...
            {/* Direct Project ZIP Download & Mobile Export Section */}
            <div className="p-4.5 bg-gradient-to-br from-amber-500/15 via-emerald-500/10 to-teal-500/15 rounded-2xl border border-amber-500/35 space-y-3 shadow-xs">
              <div className="flex items-center gap-2.5">
                <span className="p-2.5 bg-amber-500/20 text-amber-600 dark:text-amber-400 rounded-xl text-base shadow-xs">
                  📦
                </span>
                <div>
                  <h4 className="text-xs sm:text-sm font-black text-slate-900 dark:text-white">
                    {settings.language === 'en' ? 'Export & Download Complete Project' : 'تصدير وتحميل التطبيق بالكامل (ZIP)'}
                  </h4>
                  <p className="text-[10px] sm:text-[11px] text-slate-600 dark:text-slate-300">
                    {settings.language === 'en'
                      ? 'Fully updated, ready for Android Studio, Xcode, iOS & Web hosting'
                      : 'محدث بالكامل وجاهز للتحويل إلى APK أندرويد و iOS ورفع الويب'}
                  </p>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
                {/* 1. Complete Source Package */}
                <a
                  href="/noor-islam-latest-source.zip"
                  download="noor-islam-latest-source.zip"
                  className="py-2.5 px-3.5 bg-emerald-600 hover:bg-emerald-700 active:scale-98 text-white font-extrabold text-xs rounded-xl shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer no-underline text-center border border-emerald-500/30"
                >
                  <span>💻 {settings.language === 'en' ? 'Source Code ZIP' : 'الكود المصدري كاملاً (Source)'}</span>
                  <span>⬇️</span>
                </a>

                {/* 2. Production Ready Bundle */}
                <a
                  href="/noor-islam-production-build.zip"
                  download="noor-islam-production-build.zip"
                  className="py-2.5 px-3.5 bg-amber-500 hover:bg-amber-600 active:scale-98 text-slate-950 font-extrabold text-xs rounded-xl shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer no-underline text-center border border-amber-400"
                >
                  <span>🚀 {settings.language === 'en' ? 'Production Build ZIP' : 'حزمة الإنتاج الجاهزة (Build)'}</span>
                  <span>⬇️</span>
                </a>
              </div>
            </div>
