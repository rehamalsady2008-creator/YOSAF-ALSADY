  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[100] bg-gradient-to-br from-slate-950 via-emerald-950 to-slate-900 text-white flex flex-col items-center justify-between px-6 sm:px-10 select-none overflow-y-auto"
        style={{
      ...
      >
        ...
        {/* Top Header / Bismillah - Lowered with Comfortable Padding for iOS & Android */}
        <motion.div
          initial={{ y: -20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ duration: 0.6 }}
          className="text-center z-10 space-y-2.5 pt-3 sm:pt-4"
        >
          <div className="inline-flex items-center justify-center px-5 py-2 rounded-2xl bg-white/10 border border-amber-400/30 text-sm sm:text-base font-bold text-amber-300 font-kufi shadow-lg backdrop-blur-md">
            ﷽
          </div>
          <p className="text-xs sm:text-sm text-emerald-200/90 font-sans tracking-wide drop-shadow-xs">
