...
export default function ProfileSection({
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-6 text-right font-sans" dir="rtl">
      ...
      {/* 3D Realistic Profile Hero Header Card */}
      <div className="p-5 sm:p-7 bg-gradient-to-r from-emerald-900 via-teal-950 to-[#0A1A1C] text-white rounded-3xl shadow-lg border border-emerald-500/30 flex flex-col sm:flex-row items-center justify-between gap-5 relative overflow-hidden">
        {/* Background Islamic Motif Glow */}
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4.5 text-center sm:text-right relative z-10">
          {/* 3D Realistic Avatar with Gold Bezel */}
          <div className="relative w-20 h-20 sm:w-22 sm:h-22 rounded-full p-1 bg-gradient-to-tr from-amber-400 via-emerald-400 to-amber-300 shadow-md shrink-0">
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-center sm:justify-start gap-2 flex-wrap">
              <h2 className="text-xl sm:text-2xl font-black font-kufi text-emerald-200">
                {currentUser?.displayName || (isEn ? "Honored Believer" : "المستخدم الكريم")}
              </h2>
              <span className="px-2.5 py-0.5 rounded-full text-[11px] font-extrabold bg-amber-400/20 text-amber-300 border border-amber-400/30">
                {isGuest ? (isEn ? "Guest Mode" : "وضع الزائر") : (isEn ? "Verified Member" : "عضو مسجل")}
              </span>
            </div>
            <p className="text-xs text-emerald-200/70 font-mono">
              {currentUser?.email || (isEn ? "Guest Account (Local Data Saved)" : "حساب محلي محفوظ بالكامل")}
            </p>
            <p className="text-xs text-amber-200/90 font-medium pt-0.5">
              ✨ {isEn ? "Daily prayers & deeds tracker active" : "متابعة الطاعات والأذكار اليومية مفعلة"}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 relative z-10 shrink-0">
          <button
            onClick={onLogout}
            className="px-4 py-2.5 bg-rose-500/20 hover:bg-rose-500/30 text-rose-200 border border-rose-500/40 rounded-2xl text-xs font-black transition-all flex items-center gap-1.5 cursor-pointer active:scale-95 shadow-xs"
          >
